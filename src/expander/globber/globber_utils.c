...
/*   Updated: 2023/02/28 21:28:18 by lorbke           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	**add_unglobbed_vars(char **exp_argv, char **argv)
{
	...
	if (exp_argv != NULL)
	{
		exp_argv = realloc_string_array(exp_argv, 1);
		i = get_string_array_len(exp_argv);
	}
	else
...
static char	**add_globbed_vars(char **exp_argv, char **result)
{
	...
	else
	{
		...
		exp_argv = realloc_string_array(exp_argv, i);
		i = get_string_array_len(exp_argv);
		while (result[j] != NULL)
		{
