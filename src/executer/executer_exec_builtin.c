...
/*   Updated: 2023/02/25 17:15:50 by lorbke           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static pid_t	case_pipe(t_cmd_table *cmd_table, int fd_pipe)
{
	pid_t	pid;
	int		status;
...
static pid_t	case_not_pipe(t_cmd_table *cmd_table)
{
	int		fd_temp;
	int		status;
...
pid_t	exec_builtin(t_cmd_table *cmd_table, int fd_pipe)
{
	...
	if (cmd_table->fd_in[1] != FDLVL_PIPE
		...
		pid = case_not_pipe(cmd_table);
	else
		pid = case_pipe(cmd_table, fd_pipe);
	return (pid);
}
